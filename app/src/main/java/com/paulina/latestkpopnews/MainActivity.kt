        ...
        setContent {
            LatestkpopnewsTheme {
                Surface(
                    ...
                    MainScreen(navController) // Pass NavController to MainScreen
                }
            }
        }
    }
...
fun MainScreen(navController: NavHostController) {
    ...
    Column {
        ...
        when (selectedTabIndex) {
            0 -> LatestKpopNewsApp(navController) // Pass NavController
            1 -> KpopGroupsApp()
        }
    }
}
...
@Composable
fun LatestKpopNewsApp(navController: NavHostController) {
    val latestKpopNewsList = Datasource().loadLatestKpopNews()
    LatestKpopNewsList(latestKpopNewsList = latestKpopNewsList, navController = navController)
}

@Composable
fun LatestKpopNewsList(latestKpopNewsList: List<LatestKpopNews>, navController: NavHostController, modifier: Modifier = Modifier) {
    LazyColumn(modifier = modifier) {
        items(latestKpopNewsList) { news ->
            LatestKpopNewsCard(
                news = news,
                modifier = Modifier.padding(8.dp),
                onClick = { navController.navigate("newsDetail/${news.stringResourceId}") } // Example navigation
            )
        }
    }
}
...
fun DefaultPreview() {
    LatestkpopnewsTheme {
        MainScreen(rememberNavController()) // Pass a mock NavController for preview
    }
}